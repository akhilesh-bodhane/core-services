...
public class GatewayService {
	...
	public Set<String> getActiveGateways() {
		return GATEWAY_MAP.entrySet().stream().filter(gateway -> gateway.getValue().isActive() == true)
	 ...
	 *
	 * @param transaction
	 *            Txn for which payment should be initiated
	 ...
	 * @param params
	 *            Response params sent by the gateway
	...
	RefundTransaction getLiveRefundStatus(RefundTransaction currentStatus) {
		Gateway gateway = getGateway(currentStatus.getGateway());
		return gateway.fetchRefundStatus(currentStatus);
	}
	...
	public RefundTransaction initiateRefundTxn(RefundTransaction transaction) {
		if (!isGatewayActive(transaction.getGateway()))
			throw new CustomException("INVALID_PAYMENT_GATEWAY", "Invalid or inactive payment gateway provided");

		Gateway gateway = getGateway(transaction.getGateway());
		return gateway.initiateRefund(transaction);
	}
